...
library(pacman)
p_load(tidyverse,lme4,pastecs,MuMIn,lmerTest)
library(lmerTest)
...
data_check<-summary(df_train)
data_check

V1<-filter(df_train,VISIT==1)
V1%>%group_by(Diagnosis)%>%summarize("Number"=n(),"meanAge"=mean(Age,NA.RM=T),"FemaleN"=sum(Gender=="F"),"meanVIQ"=mean(VerbalIQ,NA.RM=T),"meanNVIQ"=mean(nonVerbalIQ,NA.RM=T),"SD_Age"=sd(Age))
...
inter_model<-lmer(CHI_MLU~VISIT*Diagnosis+(1+VISIT|SUBJ),df_train)
model<-lmer(CHI_MLU~VISIT+Diagnosis+(1+VISIT|SUBJ),df_train)

anova(inter_model,model)
```

How would you evaluate whether the model is a good model?

```{r ex2 evaluate, include = FALSE}
r.squaredGLMM(inter_model)
#FIXED effects 0.35, #ALSO RANDOM 0.81
```
...
```{r}
ggplot(df_train,aes(CHI_MLU,fitted(inter_model)))+geom_point()+geom_smooth(method="lm")
...
```{r ex3, include = FALSE}
int<-lmer(MOT_MLU~VISIT*Diagnosis+(1+VISIT|SUBJ),data=df_train)

summary(int)

noint<-lmer(MOT_MLU~VISIT+Diagnosis+(1+VISIT|SUBJ),data=df_train)
...
ggplot(df_train,aes(MOT_MLU,fitted(noint)))+geom_point()+geom_smooth(method="lm")

ggplot(df_train, aes(VISIT, MOT_MLU, colour = Diagnosis)) + stat_summary(fun.y = mean, geom = "point") + stat_summary(fun.y = mean, geom = "line", aes(group=Diagnosis))+ geom_smooth(method = lm, se=F) + labs(x="Visit", y="Mean Length of Utterances Mother")+ ggtitle("Use of language: parents of ASD Compared to TD children")
```

Parent MLU is affected by diagnosis and visit but probably not an interaction between diagnosis and visit
[REPORT THE RESULTS]
...
```{r ex4, include = FALSE}
summary(lmer(CHI_MLU~VISIT*Diagnosis+(1+VISIT|SUBJ),df_train))

soc<-lmer(CHI_MLU~VISIT*Diagnosis+Socialization+VerbalIQ1+(1+VISIT|SUBJ),df_train)



p_load(MASS)
fit <- lmer(CHI_MLU~Diagnosis*VISIT+Ethnicity+Gender+VerbalIQ1+nonVerbalIQ1+Socialization1+(1+VISIT|SUBJ),data=df_train)
